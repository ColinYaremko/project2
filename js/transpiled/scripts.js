...
$(function () {
  ...
  $('select').on('change', function () {
    ...
    $.ajax({
      ...
      //removed function to es6

      if (status.num_results === 0) {
        //if num_results is 0, display message.
        ...
        var nytData = result.results.filter(function (item) {
          //filter for pic *removed function

          if (item.multimedia.length !== 0) {

            return item.multimedia.length;
          }
        }).slice(0, 12).forEach(function (key, index) {
          //slice past 12 *removed function
